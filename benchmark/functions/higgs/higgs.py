...
from pdfflow.configflow import float_me, fone, fzero, DTYPE, DTYPEINT
from pdfflow.functions import _condition_to_idx
from vegasflow.vflow import VegasFlow

import tensorflow as tf

from benchmark.functions.higgs.parameters import *
from benchmark.functions.higgs import phase_space
from benchmark.functions.higgs import spinors
from benchmark.functions.higgs import me
...
@tf.function
def vfh_production_leading_order(xarr,n_dim=None,weight=None, **kwargs):
...
@tf.function
def vfh_production_real(xarr,n_dim=None,weight=None, **kwargs):
...
@tf.function
def vfh_production_nlo(xarr,n_dim=None,weight=None, **kwargs):
...
@tf.function(input_signature=[
                   tf.TensorSpec(shape=[None,6], dtype=DTYPE),
                   tf.TensorSpec(shape=[], dtype=DTYPEINT),
                  tf.TensorSpec(shape=[None], dtype=DTYPE)
               ]
           )
def vfh_production_leading_order1(xarr,n_dim=None,weight=None, **kwargs):
    """ Wrapper for LO VFH calculation
    """
    # Compute the phase space point
    pa, pb, p1, p2, x1, x2, wgt = phase_space.psgen_2to3(xarr)
    ...
    res = lumi * me_lo * wgt
    final_result = res * flux / x1 / x2
    return tf.scatter_nd(idx, final_result, tf.shape(xarr)[0:1])

@tf.function(input_signature=[
                   tf.TensorSpec(shape=[None,9], dtype=DTYPE),
                   tf.TensorSpec(shape=[], dtype=DTYPEINT),
                  tf.TensorSpec(shape=[None], dtype=DTYPE)
               ]
           )
def vfh_production_real1(xarr,n_dim=None,weight=None, **kwargs):
    """ Wrapper for R VFH calculation
    """
    # Compute the phase space point
    pa, pb, p1, p2, p3, x1, x2, wgt = phase_space.psgen_2to4(xarr)
    ...
    max_pt2 = tf.boolean_mask(max_pt2, stripe, axis=0)
    if phase_space.UNIT_PHASE:
        return tf.scatter_nd(idx, wgt, shape=tf.shape(xarr)[0:1])
    ...
    #     res = lumi * wgt
    final_result = res * flux / x1 / x2
    return tf.scatter_nd(idx, final_result, shape=tf.shape(xarr)[0:1])
...
def vfh_production_nlo1(xarr,n_dim=None,weight=None, **kwargs):
    """ Wrapper for R VFH calculation at NLO (2 jets)
    """
    # Compute the phase space point
    pa, pb, p1, p2, p3, x1, x2, wgt = phase_space.psgen_2to4(xarr)
    ...
    lumi = luminosity(x1, x2, max_pt2)
    phys_me = lumi * me_r * wgt / x1 / x2
    phys_res = tf.scatter_nd(idx, phys_me, shape=tf.shape(xarr)[0:1])
    ...
    if SUBTRACT:
        ...
        # Add this to the result
        phys_res -= tf.scatter_nd(idx, sub_1, shape=tf.shape(xarr)[0:1])
        ...
        sub_2 = lumi_2 * dip_2 * red_2 * me.factor_re * wgt_2 / x1 / x2
        # Add this to the result
        phys_res -= tf.scatter_nd(idx, sub_2, shape=tf.shape(xarr)[0:1])
